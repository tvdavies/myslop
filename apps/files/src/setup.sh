...
persist() {
  ...
  # Persist MYSLOP_FILES_TOKEN for common shells. The token file above is the
...
FISH
  ...
  say ""
  say "✓ done — MYSLOP_FILES_TOKEN is set in new shells (token: $TOKEN_FILE)"
  say "  this shell: export MYSLOP_FILES_TOKEN=\"\$(cat $TOKEN_FILE)\""
}
...
while :; do
  ...
  case "$TOKEN" in
    msf_?*|msa_?*) ;;
    *) say "  not an msf_ or msa_ token — try again"; continue ;;
  esac
