#!/usr/bin/env bash
# myslop-plans client setup: run with
#   curl -fsS https://plans.myslop.app/setup.sh | bash
set -euo pipefail

BASE="https://plans.myslop.app"
CONFIG_DIR="${XDG_CONFIG_HOME:-$HOME/.config}/myslop-plans"
...
persist() {
  ...
  # Persist MYSLOP_PLANS_TOKEN for common shells. The token file above is the
  # shell-agnostic source of truth; tools can always read it directly.
  EXPORT_LINE='export MYSLOP_PLANS_TOKEN="$(cat "${XDG_CONFIG_HOME:-$HOME/.config}/myslop-plans/token")"'
  MARKER="# myslop-plans token"
  WROTE_RC=0
  ...
  if [ -d "${XDG_CONFIG_HOME:-$HOME/.config}/fish" ]; then
    FISH_CONF="${XDG_CONFIG_HOME:-$HOME/.config}/fish/conf.d/myslop-plans.fish"
    mkdir -p "$(dirname "$FISH_CONF")"
    cat >"$FISH_CONF" <<'FISH'
# myslop-plans token
set -l _msp_dir $HOME/.config
test -n "$XDG_CONFIG_HOME"; and set _msp_dir $XDG_CONFIG_HOME
if test -r $_msp_dir/myslop-plans/token
    set -gx MYSLOP_PLANS_TOKEN (cat $_msp_dir/myslop-plans/token)
end
FISH
  fi

  say ""
  say "✓ done — MYSLOP_PLANS_TOKEN is set in new shells (token: $TOKEN_FILE)"
  say "  this shell: export MYSLOP_PLANS_TOKEN=\"\$(cat $TOKEN_FILE)\""
}
...
while :; do
  ...
  case "$TOKEN" in
    msp_?*|msa_?*) ;;
    *) say "  not an msp_ or msa_ token — try again"; continue ;;
  esac
